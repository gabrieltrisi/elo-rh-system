...
const normalizeTrainings = (items) =>
  (items || []).map((item) => ({
    id: item.id,
    employeeId: item.employeeId || item.employee?.id || null,
    status: (item.status || '').toUpperCase(),
...
const Onboarding = () => {
  ...
  const fetchUniforms = async () => {
    try {
      const res = await api.get('/uniform-deliveries');
      setUniforms(normalizeUniforms(res.data?.deliveries || []));
    } catch (error) {
  ...
  const fetchTrainings = async () => {
    try {
      ...
      setTrainings(normalizeTrainings(res.data?.trainings || []));
    } catch (error) {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'status') {
      setFormData((prev) => ({
        ...
        completedAt:
          value === 'CONCLUIDO'
            ? prev.completedAt || getTodayDateInputValue()
            : '',
      }));
      return;
    }
    ...
    if (name === 'startDate') {
      setFormData((prev) => {
        ...
        if (
          ...
          next.completedAt &&
          value &&
          new Date(next.completedAt) < new Date(value)
        ) {
  ...
  const handleCreateOrUpdate = async (e) => {
    ...
    if (
      ...
      formData.startDate &&
      new Date(formData.completedAt) < new Date(formData.startDate)
    ) {
    ...
    const payload = {
      ...
      completedAt:
        formData.status === 'CONCLUIDO' ? formData.completedAt || null : null,
  ...
  const onboardingCards = useMemo(() => {
    return onboardings.map((item) => {
      ...
      const employeeTrainings = trainings.filter(
        (training) => Number(training.employeeId) === Number(item.employeeId)
      );

      const completedTrainings = employeeTrainings.filter(
        (training) => training.status === 'CONCLUIDO'
      );

      const trainingReady = completedTrainings.length > 0;
  ...
  }, [onboardings, documents, uniforms, benefits, trainings]);
            ...
            <div className='space-y-5 p-5'>
              ...
              <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                ...
                <div className='rounded-2xl border border-slate-200 bg-slate-50 p-4'>
                  ...
                  <p className='mt-1 text-xs text-slate-500'>
                    {item.trainingReady
                      ? 'Capacitacoes concluidas e rastreadas'
                      : 'Nenhum treinamento concluido ainda'}
                  </p>
                </div>
              </div>
                    ...
                    <div className='grid grid-cols-1 gap-5 md:grid-cols-2'>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className='w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none transition focus:border-slate-500 disabled:cursor-not-allowed disabled:bg-slate-100'
                        />
                        <p className='mt-2 text-xs text-slate-500'>
                          Preenchida automaticamente quando o status for
                          concluido.
                        </p>
                      </div>
                    </div>
