...
const Suspensions = () => {
  ...
  const fetchSuspensions = async () => {
    try {
      ...
      const rawSuspensions = res.data?.suspensions || [];
      setSuspensions(rawSuspensions.map(mapSuspensionFromApi));
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.error('Erro ao excluir suspensão:', error);
      alert(error?.response?.data?.message || 'Erro ao excluir suspensão.');
    }
  };

  const handleOfficialPdf = async (suspension, mode = 'view') => {
    const actionKey = `${mode}-${suspension.id}`;
    ...
    try {
      ...
      const response = await api.post(`/pdf/suspensions/${suspension.id}`);
      ...
      if (mode === 'download') {
        await downloadPdfFromEndpoint(
          pdf.downloadUrl,
          `suspensao-${suspension.id}-${suspension.employeeName}.pdf`
        );
        return;
      }
      ...
      console.error('Erro ao gerar PDF da suspensao:', error);
  ...
  const stats = useMemo(() => {
    return {
      ...
      registered: suspensions.filter(
        (item) => (item.status || '').toLowerCase() === 'registrada'
      ).length,
      active: suspensions.filter(
        (item) => (item.status || '').toLowerCase() === 'ativa'
      ).length,
      finished: suspensions.filter(
        (item) => (item.status || '').toLowerCase() === 'finalizada'
      ).length,
    };
  }, [suspensions]);
          ...
          >
            <div className='flex flex-col gap-4'>
              ...
              <div className='flex flex-wrap gap-2 pt-2'>
                <button
                  type='button'
                  onClick={() => handleOfficialPdf(item, 'view')}
                ...
                >
                  {pdfActionKey === `view-${item.id}`
                    ? 'Gerando...'
                    : 'Gerar documento oficial'}
                </button>
                <button
                  type='button'
                  onClick={() => handleOfficialPdf(item, 'download')}
                  className='rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50'
                >
                  {pdfActionKey === `download-${item.id}` ? 'Baixando...' : 'Baixar PDF'}
                </button>
                <button
                  type='button'
                  ...
                  <tr key={item.id} className='hover:bg-slate-50/70'>
                    ...
                    <td className='px-6 py-5'>
                      <div className='flex flex-wrap justify-center gap-2'>
                        ...
                        >
                          {pdfActionKey === `view-${item.id}` ? 'Gerando...' : 'PDF'}
                        </button>
                        <button
                          type='button'
                          onClick={() => handleOfficialPdf(item, 'download')}
                ...
                <div className='space-y-6'>
                  <section className='rounded-2xl border border-slate-200 bg-white p-5 shadow-sm'>
                    <div className='mb-5'>
                      ...
                      <p className='mt-1 text-sm text-slate-500'>
                        Selecione o colaborador relacionado à suspensão.
                      </p>
                    </div>
