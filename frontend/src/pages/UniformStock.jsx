...
const UniformStock = () => {
  ...
  const loadStock = async () => {
    try {
      const response = await api.get('/uniform-stock');
      ...
      console.error('Erro ao buscar estoque de fardamento:', error);
      setStock([]);
    }
  };
  ...
  const loadDeliveries = async () => {
    try {
      const response = await api.get('/uniform-deliveries');
      const items = response.data?.deliveries || [];

      setDeliveries(
        items.map((item) => ({
          id: item.id,
          employeeId: item.employeeId,
          employeeName: item.employee?.name || item.employee?.fullName || '',
          sector: item.uniformStock?.sector || '',
          itemLabel: buildItemLabel({
            type: item.uniformStock?.itemName || '',
            color: item.uniformStock?.color || '',
            size: item.uniformStock?.size || '',
          }),
          itemType: item.uniformStock?.itemName || '',
          itemColor: item.uniformStock?.color || '',
          itemSize: item.uniformStock?.size || '',
          quantity: Number(item.quantity || 0),
          date: item.deliveryDate || item.createdAt || '',
          notes: item.notes || '',
          uniformStockId: item.uniformStockId,
        }))
      );
    } catch (error) {
      console.error('Erro ao buscar entregas de fardamento:', error);
      setDeliveries([]);
    }
  };
  ...
  const handleCreateItem = async (e) => {
    ...
    try {
      await api.post('/uniform-stock', {
        sector: itemForm.sector,
        itemName: itemForm.type,
        color: itemForm.color || '',
        size: itemForm.size,
        availableQuantity: Number(itemForm.quantity),
      });

      await loadStock();
      closeCreateDrawer();
    } catch (error) {
      console.error('Erro ao salvar item de fardamento:', error);
      alert(
        error?.response?.data?.message ||
          'Não foi possível salvar o item de fardamento.'
      );
    }
  };

  const handleDeliverUniform = async (e) => {
    e.preventDefault();
    ...
    try {
      await api.post('/uniform-deliveries', {
        employeeId: Number(deliveryForm.employeeId),
        uniformStockId: Number(selectedItem.id),
        quantity: quantityToDeliver,
        notes: deliveryForm.notes,
      });

      await Promise.all([loadStock(), loadDeliveries()]);
      closeDeliveryDrawer();
      setActiveTab('deliveries');
    } catch (error) {
      console.error('Erro ao registrar entrega de fardamento:', error);
      alert(
        error?.response?.data?.message ||
          'Não foi possível registrar a entrega de fardamento.'
      );
    }
  };
