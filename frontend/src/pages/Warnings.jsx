...
const Warnings = () => {
  ...
  const handleOfficialPdf = async (warning, mode = 'view') => {
    const actionKey = `${mode}-${warning.id}`;
    ...
    try {
      ...
      const response = await api.post(`/pdf/warnings/${warning.id}`);
      ...
      if (mode === 'download') {
        await downloadPdfFromEndpoint(
          pdf.downloadUrl,
          `advertencia-${warning.id}-${warning.employeeName}.pdf`
        );
        return;
      }
      ...
      console.error('Erro ao gerar PDF da advertencia:', error);
